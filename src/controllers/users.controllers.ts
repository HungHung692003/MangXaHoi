...
export const forgotPasswordController = async (
  ...
  const mailOptions = {
    ...
    text: `Chào ${req.user.name}, vui lòng xác thực tài khoản của bạn bằng cách nhấp vào liên kết sau: ${process.env.CLIENT_ORIGIN}/verify-forgot-password?token=${result.forgot_password_token}`
  }
...
export const getProfileController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Gọi service để lấy thông tin người dùng và bài đăng
    const data = await usersService.getProfileWithTweets(_id)
...
// Controller để lấy thông tin danh sách follower
export const getFollowersController = async (req: CustomRequest, res: Response, next: NextFunction) => {
  try {
    const followedUserId = req.params.id
    const followers = await usersService.getFollowersWithDetails(followedUserId)
    return res.status(200).json({
...
export const unfollowController = async (req: CustomRequest<UnfollowReqParams>, res: Response) => {
  const { user_id } = req.decoded_authorization as TokenPayload
  const followed_user_ids = req.params.user_id
  const result = await usersService.unfollowe(user_id, followed_user_ids)
  return res.json(result)
}
