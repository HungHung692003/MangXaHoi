...
const TweetDetail = () => {
  ...
  const handlePost = async () => {
    try {
      let medias = []
      const res = await axiosInstance.post('/createNotification', {
        TweetId: id,
        ownerId: createNotifications.data.result.user_id,
        actionType: 'comment'
      })
      console.log('createNotifications', createNotifications.data.result.user_id)
      console.log('res', res)
      socket.emit('getNotifications', { user_id: res.data.data.ownerId })
