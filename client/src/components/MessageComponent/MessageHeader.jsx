...
export default function MessageHeader({ onClickMessage, senderId }) {
  ...
  useEffect(() => {
    const fetchFollowers = async () => {
      try {
        const res = await axiosInstance.get(`http://localhost:3000/api/followers/${senderId}`)
        const followers = res.data.result.map((follower) => ({
          id: follower._id, // Lấy trực tiếp từ API không lặp
          name: follower.name,
          avatar: follower.avatar || 'https://via.placeholder.com/40', // Ảnh mặc định nếu không có avatar
